...
/**
 * A match for the rule <code>oscBwd()</code>.
 */
public class OscBwdMatch extends GraphTransformationMatch<OscBwdMatch, OscBwdRule> {
	private U varU;
	private V varV;
	private W varW;
	private X varX;
	...
	/**
	 * Creates a new match for the rule <code>oscBwd()</code>.
	 * 
	 * @param pattern
	 *            the pattern
	 * @param match
	 *            the untyped match
	 */
	public OscBwdMatch(final OscBwdRule pattern, final IMatch match) {
		super(pattern, match);
		varU = (U) match.get("u");
		varV = (V) match.get("v");
		varW = (W) match.get("w");
