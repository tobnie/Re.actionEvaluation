...
public class P_s_object extends AbstractActor {
	...
	private List<Port<GKL800Model.P_s>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<GKL800Model.P_s>(getSelf(), name2actor.get("T_T_x_p_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<GKL800Model.P_s>(getSelf(), name2actor.get("T_T_y_p_0_reference"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKL800Model.P_s>(getSelf(), name2actor.get("Tp_x_31_junction"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKL800Model.P_s>(getSelf(), name2actor.get("Tp_y_45_junction"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<GKL800Model.P_s> message) {
		for(Port<GKL800Model.P_s> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<GKL800Model.P_s> message) {
		for(Port<GKL800Model.P_s> port : ports) {
	...
	private void changeAttribute(AttributeChanged<GKL800Model.P_s> message) {
		for(Port<?> port : ports) {
